...
module "alb" {
  ...
  target_groups            = "${list(map("name", "${var.project_name}-web", "backend_protocol", "HTTP", "backend_port", "80", "health_check_path", "/heartbeat"), map("name", "${var.project_name}-admin", "backend_protocol", "HTTP", "backend_port", "80", "health_check_path", "/in.php"))}"
